#!/usr/bin/env python3
#
# --- Test script for the semi-implicit Darwin solver in WarpX, wherein both
# --- ions and electrons are treated as kinetic particles. The script
# --- demonstrates the use of this model to simulate magnetic reconnection in
# --- a force-free sheet. The setup is based on the problem described in Le
# --- et al. (2016) https://aip.scitation.org/doi/10.1063/1.4943893.
# ---
# --- The CI (--test) configuration only runs a handful of steps at reduced
# --- resolution: this is a smoke test exercising the solver (and its
# --- pc_darwin_mlmg preconditioner - see DarwinMLMGPC.H - which drops the
# --- GMRES iteration count roughly 50x versus the unpreconditioned solve on
# --- this problem), not a physics validation of reconnection onset, which
# --- requires the much longer, full-resolution run below.
...
class ForceFreeSheetReconnection(object):
    ...
    def __init__(self, test, verbose):
        ...
        # Run very low resolution, few steps, and few particles per cell as a
        # CI smoke test - this only checks the solver runs, converges (an
        # unconverged GMRES solve aborts the whole run - see
        # WarpX::OneStep()), and produces finite, bounded fields; it does not
        # attempt to resolve reconnection onset.
        if self.test:
            self.total_steps = 8
            self.diag_steps = 4
            self.NX = 64
    ...
    def setup_run(self):
        ...
        simulation.evolve_scheme = picmi.SemiImplicitDarwinEvolveScheme(
            linear_solver=picmi.GMRESLinearSolver(
                ...
                max_iterations=2048,
                restart_length=512,
                verbose_int=(2 if self.test else 1),
        ...
        self.solver = picmi.ElectrostaticSolver(
            ...
            # warpx_skip_domain_bc_value_update=True
        )
    ...
    def check_fields(self):
        ...
        Jy = simulation.fields.get("current_fp", dir="y", level=0)[...] / self.J0
        ...
        with open(f"diags/fields/fields_{step:06d}.npz", "wb") as f:
            np.savez(f, rho=rho, Jy=Jy, Bx=Bx, By=By, Bz=Bz)
